"""
Cell Transport Potential for SC53 tumors
...
rep1 = scv.read(f'SC53/{sample1}.loom', cache=True)
rep1.var_names_make_unique()

#treated SC53 sample
sample2 = 'SC53cis.LB19006_GT19_34513_SI-GA-B1_S2_L001'
rep2 = scv.read(f'SC53/{sample2}.loom', cache=True)
...
unt_names = pd.read_csv('./SC53/53_unt_filtered_cells.csv', header = 0, index_col=0)
unt_filtered = []
for i in unt_names['x']:
    n = i.split('.')[1].split('-')[0]
    unt_filtered.append(n)

cis_names = pd.read_csv('./SC53/53_cis_filtered_cells.csv', header = 0, index_col=0)
cis_filtered = []
...
scv.pl.paga(adata, basis='tsne', size=50, alpha=.1, palette = 'tab20',
            min_edge_width=2, node_size_scale=1.5, save = 'paga_all_53')
scv.pl.paga(unt, basis='tsne', size=50, alpha=.1,palette = 'tab20',
            min_edge_width=2, node_size_scale=1.5,save = 'paga_unt_53')
scv.pl.paga(cis, basis='tsne', size=50, alpha=.1,palette = 'tab20',
            min_edge_width=2, node_size_scale=1.5,save = 'paga_cis_53')
